...
- **CPU a 1,59 GHz**, no a 2,13: el kernel dice *"Unable to measure TSC frequency, assuming default"* (1594 MHz) y el *uptime* cuadra con el reloj real, así que la frecuencia es esa de verdad. **Resuelto el 14-09:** es el P-state P2 que deja el arranque; el kernel no tiene `cpufreq` y nunca pide P0 (2,1 GHz), que está permitido. [`cpu/ps4-cpu`](cpu/README.md) lo pide por MSR, con dos iconos en el escritorio; hay que pulsarlo en cada arranque. Pregunta abierta solo para `neocine-1.1` y rmux: si traen `cpufreq`, sobra.
